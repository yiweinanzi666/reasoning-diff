...
独立性声明：未阅读 `.planning/audits/round-23/` 下除 `VERSION.md` 与本文件外的其他通道报告。未把 `.planning/audits/round-22/{A,B,C,D,E,F}-*.md` 当作本树证据。只把 `VERSION.md` 与 `.planning/audits/ISSUES.md` 当作冻结声明 / 作者台账，不把作者 `local close` 或 r22 通过结论当关闭。对照读了本通道 round-21 报告仅作版式与猎单结构，不作本轮证据。未修改 `src/`、`tests/` 或 `pyproject.toml`。绿测试不是论文正确性。微型随机权重是本地接口证明，**不是** MODEL-01。Gate 未注册与 `scientific_conclusion=None` 不记缺陷。约束 `\nq = <digit>` **不是** §4.1 自然 CoT。作者 F20-02（`cmd_calibrate` 兄弟 `lab`）与 F19-06（scientific fit §8 行）不在本猎单，不裁定。

对照原文：论文 §2.4（\(H'=H_b+\Pi_Z(H_d-H_b)\)、C-rand/C-layer 同批次）、§4.1（自然 CoT，本机不得冒称）、§4.2 / 第 5–6 节（三位置；前瞻交换）、第 6/8 节（来源—数值 / Knock-out / Rescue）；GOAL MODEL-01 / CAUSAL-01 / CAUSAL-02 / POS-01 / INLP-01 / IE-01 / EXEC-01 / REPAIR-01；`docs/EXPERIMENT_PROTOCOL.md` §4；GOAL §5.15（普通子进程不得冒称隔离沙箱）；GOAL §5.9（卡片 hidden 4096/3584）。tiny 路径只验收接口与接线，不验收 MODEL-01。

**先行结论：** 按 `.planning/audits/round-23/VERSION.md` 原文脚本复算，开审与交卷均为 **HASH_MATCH** `1f5f379835bba8431e1857c7d184ba4fc136bf1896a15b3193a5609819c7bcdb`（61 文件，0 CRLF）。指定 scientific 路径在无 `prep` 兄弟的 `stage_a`/`stage_b` 上独立复跑。70 字题干的独立三路间谍（`intervene_hidden_decode` 的 `prompt_ids`、`decode_loop` 张量宽、`resid_post` 首次前向 `seq`）均为 **79**，全部 >64，无一被切到 64；落盘 `prefix_n=79` 与间谍一致。97 token 前缀与 97/127 字问句 raise。点名猎项全部成立。未发现需改冻结集的本通道缺陷。
...
| 审查时间 | 2026-09-21（Asia/Shanghai）；开审约 03:48，交卷约 04:15 |
| 声称冻结 hash | `1f5f379835bba8431e1857c7d184ba4fc136bf1896a15b3193a5609819c7bcdb`（`.planning/audits/round-23/VERSION.md`；61 文件；POSIX relpath + NUL + bytes） |
...
| 作者 pytest 声称 | 172 passed。本机全量 **172 passed / 22.01s / exit 0**。绿只锁回归，不是论文正确性 |
| 明确未读 | `.planning/audits/round-23/` 下除 `VERSION.md` 与本文件外的其他通道报告；未把 r22 A–F 报告当本树证据 |

因果生产文件 SHA-256（交卷 = 开审）：`cli.py` `cc5eae53d7be27eb`（61734 B / 1268 行）、`edits.py` `cc8e2d23592a3ab8`（13905 B / 367）、`collect.py` `83658cd8ba879f20`（261）、`generate.py` `6e6040394ac641c7`（200）、`tiny.py` `c75d0f5325766612`（120）、`features.py` `0a9f0b8beb0185ae`（39）、`adapters.py` `c1992624026a1bf0`（42）、`tokenize.py` `b0cc8974af1010fa`（31）、`interventions.py` `0ffdbb8805be7649`（115）、`repair.py` `f76ff9998b9a6b17`（232）、`events.py` `290a4fd676ac0814`（274）、`executor.py` `481d6ed597c93e78`（109）、`rng.py` `2098c2c72a852eaa`（53）。全部 0 CRLF。本通道对当前磁盘独立复跑。

复跑目录（本通道自建）：`.planning/audits/round-23/_d_scratch/{stage_a,stage_b,stage_int,stage_int_dev,stage_rep,stage_off,stage_no_edits,stage_70a,stage_70b,stage_70int}`。兄弟名只有这些加 `hunts.py` / `hunts.json` / `t1_70.json`；**没有** `prep` / `prepare` / `s-prep`。`stage_a` = prepare，`stage_b` = collect（`--in-dir stage_a`），intervene / repair 的 `--in-dir` 是 `stage_b`。70 字攻击用 `stage_70a`/`stage_70b`/`stage_70int`，不占用指定名。
...
| `src/reasoning_diff/models/generate.py` | 1–200 | `sample_next`；`decode_loop`（独立 `torch.Generator`）；`append_target_assignment`；`len(prompt_ids)>96` **raise**，无 `[:48]`；只解析生成区；`apply_model_template` 有定义且 **仅** `qwen3` 传 `enable_thinking`；CLI / `generate_task_trace` **无调用** |
...
支持性阅读：论文 §2.4 / 第 6、8 节；`docs/EXPERIMENT_PROTOCOL.md` §4；`.planning/REQUIREMENTS.md` 上列要求；`docs/CURSOR_GOAL_PROMPT.md` §5.9–5.15。未覆盖且不得默认通过：真实权重加载、CUDA/长链 KV、官方 tokenizer JSON、50 条来源解耦 C2 同批次、自然 CoT。未逐行审计：`analysis.py`、`probes/bilinear.py`、`transfer.py`、`measure.py`、`splits.py`（他通道）。`cli.py` 校准/fit 段属其他通道；作者 F19-06 / F20-02 校准兄弟 `lab` 不在本猎单。
...
| X2 | 全量 pytest | **172 passed / 22.01s / exit 0**，与作者数字一致。仍不是论文正确性 |
| X3 | `apply_swap` 标准正交基 | \(H_b=[1,0],H_d=[0,1],Z=e_2\) → \(H'=[1,1]\)。全空间 \(I_2\) 则 \(H'=H_d=[0,1]\) |
| X4 | tiny qwen2：`resid_post` 层 1 后 KV | 层 0/1 \(\Delta K=\Delta V=0\)；层 2 \(\Delta K_{\max}=0.166\)、\(\Delta V_{\max}=0.359\)，且 **仅 last token**（earlyK=0）。本通道自加 `+0.5` 变换 |
...
| X11 | INLP / rescue 算法 | 随机向量上 \(h@P\) 与 `apply_swap` 不是同一变换。独立 residual 间谍：`out == in @ P`（matmul 差 \(9.2\times 10^{-10}\)），`out ≠ in`（差 0.029）。库函数 `ie_z(H_3,H_0)\approx -3.07\times 10^{-4}`；**CLI tiny 路径不写该值**（写 `ie_z=0.0` / `ie_z_g=target_follow`） |
...
| X14d | 另跑 `--dev-layer-scores 0.05 0.9 0.8` | 70 字路径五次 decode 键 **层 1 / 1 / 0 / 1 / 1**，模式 **`pi_z_swap` / `add_delta`(C-rand) / `add_delta`(C-layer) / `inlp` / `replace`**。`clayer_status=dev_weak_layer_decode`，`weak_layer=0`。相对四项 `vs_crand=vs_clayer=0.0`。`donor_kind` 仍为 `same_value_diff_source`，`donor_rows=[0,3]` |
...
| X24 | INLP hook 不是 swap | 独立 residual 间谍：`out == in @ P`（matmul 差 \(9.2\times 10^{-10}\)），`out ≠ in`（差 0.029）。CLI `mode="inlp"`，无 donor。2 点拟合 `inlp_rank=30` |
...
| X26 | 问句单独解析 | 对 6 条 `prompt_text` 用**原题** `parse_events` 打出 p1/p2。对 source 问句用 **source task** 解析得 p1/`p2`/`src_b`。generate **不解析问句**，故这些不是生成事件 |
| X27 | 卡片 revision / hidden | `qwen3-8b` hidden **4096**，revision `b968826d9c…`，think `(151667,151668)`；`r1-distill-qwen-7b` hidden **3584**，revision `916b56a4…`，think `(151648,151649)`。本机未加载。缺名 `card("no-such-model")` 抛 `KeyError`。两卡 `revision ≠ latest`。内存注入 `revision="latest"` → `ValueError: mutable latest revision is forbidden` |
| X28 | collect 复制 edits | `stage_b/edits.jsonl` 存在且与 `stage_a/edits.jsonl` **逐字节相同**。intervene 只读 `stage_b` |
| X29 | `apply_model_template` 开关 | 假 tokenizer 记录 kwargs：`model_kind=qwen3` **有** `enable_thinking`；`qwen2` / `r1` **无**该键。CLI 与 `generate_task_trace` 均不调用此函数 |
...
`ISSUES.md` 只当作者主张。r22 A–F PASS 与 consecutive_pass_count=1 **不是**本通道关闭依据。
...
| C22-U-01 / F22-01 / F22-09 | fallback 旗标；绿 pytest≠Goal；Plus persist 卫生 | **不裁定 / 同意绿≠Goal。** 非本通道猎项或过程口径 |
| pending_server | 真实权重 / 50 条 C2 / 官方 CoT | **同意未关。** 约束 `q` 不是 §4.1；tiny 不是 MODEL-01 |
| Gate 未注册 | 非缺陷 | **同意。** 不记缺陷 |

## 6. 发现

### D23-01 — 无静默 generate 48 截断；超长 prompt raise
...
### D23-02 — 独立间谍 hook `prompt_ids`：70 字前缀不切开；超长 raise 不 slice
...
- **对应要求：** 点名猎项「Independently spy intervene_hidden_decode prompt_ids on a 70-char question: every call length must be >64, not silent [:64]. >96 must raise」；作者 F20-01 / F18-08（作者 close 不作关闭）
- **证据：** X14c、X22b。冻结树无 `ids[:64]` / `[:64]` / `[:48]`。
  - `_tiny_prefix_ids("a"*65)` 长度 65；`("a"*70)` 长度 70；`("a"*96)` 长度 96
  - `_tiny_prefix_ids("b"*97)` raise `tiny intervene prefix exceeds context; refuse truncated prefixes`
  - 独立 70 字夹具：原问句 36 字 pad 到 70；`stage_70a` 最短 `prompt_text=70`（source 83）；生成区 `q` 的 `start=79`
  - **三路运行时间谍（本通道自挂，不读作者测试）：**
    1. 包装 `collect.intervene_hidden_decode`：五次 `len(prompt_ids)=79`，mode/layer 为 `(pi_z_swap,1)` / `(add_delta,1)` / `(add_delta,0)` / `(inlp,1)` / `(replace,1)`
...
### D23-03 — XOR 保留 *a*、追加 *b*；重叠 rename 同时替换；配对不依赖兄弟 `prep`
...
### D23-04 — \(H\) 全有限；全 NaN 行跳过；不可表达 `start=0` 跳过
...
### D23-05 — INLP 是 \(h@P\)；C-rand `add_delta`；rescue `replace`；范数锁到主干预
...
- **证据：** X11、X12、X14c、X14d、X23、X24。residual 间谍 `out = in @ P`（matmul 差 \(9.2\times 10^{-10}\)）。五次 decode 键集合恰好 `{(pi_z_swap,1),(add_delta,1),(add_delta,0),(inlp,1),(replace,1)}`。C-rand / C-layer 范数与独立 `c_rand_delta` / `c_layer_delta` 对拍。rescue 是 `replace`，不是再 swap。落盘 `ie_z=0.0` ≠ helper \(-3.07\times 10^{-4}\)；`ie_z_g=target_follow`。本机 donor_ans=82、主/基线/对照皆为 9
...
### D23-06 — 仅生成区事件；几何 `timing` 保持 `offline_hidden`
...
### D23-07 — Prefill：真实 32-d tiny hidden 接受；标量/bool/`[0]` 拒绝
...
### D23-08 — 教师强制诚实为 `constrained_target`；不得写 MODEL-01 / §4.1
...
### D23-09 — `IsolatedExecutor` ≠ `ChildProcessExecutor`
...
### D23-10 — `apply_model_template`：`enable_thinking` 只传给 qwen3

- **状态：** closed（接口） / residual（生产未调用）
- **严重度：** n/a
- **文件 / 符号 / 行号：** `generate.py` **194–200**
- **对应要求：** 点名猎项；STACK Qwen3 vs R1 模板差异
- **证据：** X29。假 tokenizer 记录 `apply_chat_template` kwargs：`qwen3` 含 `enable_thinking=False`（本通道传入 False）；`qwen2` 与 `r1` 的 kwargs 仅有 `add_generation_prompt` / `tokenize` / `return_tensors`，**没有** `enable_thinking`。`inspect.getsource(generate_task_trace)` 与 `cli.py` 均无 `apply_model_template`
- **影响：** 指定接口不再把 thinking 开关误传给 R1/Qwen2 模板。CLI 仍不调用此函数，不是官方 CoT / MODEL-01

### D23-11 — `card()`：4096 / 3584；未知 KeyError；revision ≠ latest

- **状态：** closed（卡片数字与守卫） / residual（未 `from_pretrained`）
- **严重度：** n/a
- **文件 / 符号 / 行号：** `adapters.py` **3–29**
- **对应要求：** 点名猎项；GOAL §5.9；STACK 模型库
- **证据：** X27。`card("qwen3-8b")["hidden_size"]==4096`，revision `b968826d9c46dd6066d109eabc6255188de91218`。`card("r1-distill-qwen-7b")["hidden_size"]==3584`，revision `916b56a44061fd5cd7d6a8fb632557ed4f724f60`。`card("no-such-model")` → `KeyError`。两卡 `revision != "latest"`。内存注入 `revision="latest"` → `ValueError: mutable latest revision is forbidden`
...
| N-03 | 非缺陷 | `card()` 拒绝 `revision=="latest"`；hidden 4096/3584 |
...
| N-11 | 过程 | 开审与交卷均为 HASH_MATCH，可签在声称快照 `1f5f3798…c7bcdb` 上 |
...
开审与交卷复算冻结均为 `1f5f3798…c7bcdb`。在 `.planning/audits/round-23/_d_scratch/` 对 `tests/fixtures/t1_tiny.json` 跑 scientific `prepare --split-fractions 0.4 0.15 0.1 0.1 0.1 0.15` → `collect --backend tiny --weight-seed 0` → `intervene --backend tiny` → `intervene --dev-layer-scores 0.05 0.9 0.8` → `repair --eval-mode scientific --mask task_oracle`。目录名为 `stage_a`/`stage_b`，**没有**兄弟 `prep`。另用 pad 到 70 字的夹具跑 `stage_70a`→`stage_70b`→`stage_70int`，并自挂 hook/`decode_loop`/`resid_post` 间谍。核验：

1. **无 generate 48 截断：** source 问句 49 字符，完整 `What is q = p1 * src_b?`。`generate.py` 超 96 则 raise。
2. **无 intervene 64 截断 / 70 字 hook 间谍：** `_tiny_prefix_ids` 65/70/96 token 不切片；97 token raise。无 `ids[:64]`。70 字 CLI 三路间谍均为 79，未切到 64；落盘 `prefix_n=79`。每一次 `intervene_hidden_decode` 的 `prompt_ids` 长度均 >64。
...
12. **模板开关：** `enable_thinking` 只进入 qwen3 的 `apply_chat_template` kwargs。
13. **卡片：** `qwen3-8b` hidden 4096、`r1-distill-qwen-7b` 3584；未知名 KeyError；`revision ≠ latest`。
