...
独立性声明：未阅读 round-10 其他通道报告；只把 `.planning/audits/round-10/VERSION.md` 与 `.planning/audits/ISSUES.md` 当作冻结声明/作者台账。对照读了本通道 round-08 报告仅作版式，结论以本机磁盘与本轮复跑为准。未修改 `src/`、`tests/` 或 `pyproject.toml`。不把测试绿当作论文正确；不把作者 `local close` 或 `153 passed` 当作本通道关闭。微型随机权重是本地接口证明，**不是** MODEL-01。Gate 未注册与 `scientific_conclusion=None` 不记缺陷。
...
**先行结论：** 开审按 `VERSION.md` 原文复算，冻结 **HASH_MATCH** `813081243b89687be06260e1516c34f9d54b0217ca3f771fab9240b18f5a4b6d`（61 文件，0 CRLF）。指定 scientific 路径点名项本机全部成立：生成区事件、有限 \(H\)、`trace-base`/`trace-edit` donor、INLP \(h@P\)、C-rand/rescue/C-layer 接线、几何 `offline_hidden`、Prefill 接受/拒绝、`constrained_target` 诚实。交卷再算同一脚本为 **HASH_MISMATCH**（审查中依次见 `a536faf6…`、`ee013546…`、`7518e20b…`，写报告后最后一次为 `929aa472d78c5a1ac7dbd0a525f22cde25b6ef9964019a29c15919f56c2ecb05`；仍 61 文件）；本通道未改冻结集，因果生产文件从中检到交卷哈希未变，漂移来自并发改动。本机全量 pytest 现为 **155 passed**，与作者声称 153 不一致，**不得**绑到声明冻结。tiny 不是 MODEL-01，不得写 §4.1。
...
| 声称冻结 hash | `813081243b89687be06260e1516c34f9d54b0217ca3f771fab9240b18f5a4b6d`（`.planning/audits/round-10/VERSION.md`；61 文件；POSIX relpath + NUL + bytes） |
| 开审复算 | **HASH_MATCH。** 脚本逐字：`src/reasoning_diff/**/*.py` 排序 + `tests/**/*.py` 排序 + `pyproject.toml` → **61 文件** → `813081243b89687be06260e1516c34f9d54b0217ca3f771fab9240b18f5a4b6d` |
| 交卷复算 | **HASH_MISMATCH。** 同脚本 → 61 文件。审查中瞬时 `a536faf6…` / `ee013546…` / `7518e20b…`；写报告后最后一次 `929aa472d78c5a1ac7dbd0a525f22cde25b6ef9964019a29c15919f56c2ecb05`。0 CRLF。并发写盘，本通道未改冻结集。 |
...
| 作者 pytest 声称 | 153 passed。本机交卷全量 **155 passed / 24.45s / exit 0**。数字对不上声明冻结，不记为该 hash 的验收。因果相关 7 个文件 **63 passed / 18.22s** |
| 明确未读 | `.planning/audits/round-10/{A,B,C,E,F}-*.md`（交卷时该目录除 `VERSION.md` 外本通道只写本文件） |

因果生产文件（中检 SHA-256 前 16 = 交卷同一值）：`cli.py` `ba474e1fea1f7c5a`（1251 行）、`collect.py` `83658cd8ba879f20`（261）、`generate.py` `07493570c6410e6b`（201）、`interventions.py` `0ffdbb8805be7649`（115）、`repair.py` `f76ff9998b9a6b17`（232）、`tiny.py` `21725a183452bd06`（120）、`features.py` `0a9f0b8beb0185ae`（39）、`adapters.py` `c1992624026a1bf0`（42）、`tokenize.py` `b0cc8974af1010fa`（31）、`edits.py` `cacb63ac27cbcccf`（287）、`events.py` `290a4fd676ac0814`（274）、`executor.py` `481d6ed597c93e78`（109）、`rng.py` `2098c2c72a852eaa`（53）。科学复跑在开审 MATCH 之后立即执行；上述字节交卷未变。
...
| `src/reasoning_diff/cli.py` | 257–438、440–547、674–687、808–864、867–1010、1013–1053 | prepare/collect/pair/donor/intervene/repair |
...
| `tests/test_round04_regressions.py` | 25–137、218–226 | readout；Prefill；source_value_pair 形状 |
| `tests/test_round05_regressions.py` | 23–136 | scientific 事件；几何 ≠ `pre_step` |
| `tests/test_round06_regressions.py` | 25–88 | 字段名锁 `donor_kind`/`inlp_transform`/`ie_z_g`；**不**比 \(h@P\) 数值 |
| `tests/test_round07_regressions.py` | 39–50 | 标量/`[0]`/`[]` Prefill 拒 |

支持性：`docs/EXPERIMENT_PROTOCOL.md` §4；`.planning/REQUIREMENTS.md` MODEL-01 / CAUSAL-01 / CAUSAL-02 / POS-01 / INLP-01 / IE-01 / REPAIR-01。未逐行审计：`analysis.py`、`probes/bilinear.py`、`transfer.py`（非本通道猎项）。
...
| ID | 检查 | 结果 |
|---|---|---|
| X1 | VERSION 脚本逐字复算 | **开审 HASH_MATCH** `81308124…4b6d`，61 文件，0 CRLF。**交卷 HASH_MISMATCH**（最后一次 `929aa472…2ecb05`；中途还见 `7518e20b…` 等） |
| X2 | 因果相关 pytest 7 文件 | **63 passed / 18.22s**。绿只锁冒烟与字段名 |
| X2b | 交卷全量 pytest | **155 passed / 24.45s** ≠ 作者 153。不绑声明冻结 |
| X3 | `apply_swap` 标准正交基 | \(H_b=[1,0],H_d=[0,1],Z=e_2\) → \(H'=[1,1]\) |
| X4 | resid_post 层 1 后 KV | 层 0/1 \(\Delta K=\Delta V=0\)；层 2 \(\Delta K_{\max}=0.512\)、\(\Delta V_{\max}=0.314\)，seq=5 |
| X6 | decode hook 次数 | `once=False` **3**；`once=True` **1**（`max_new=3`） |
| X7 | hook `finally` | 正常与 `RuntimeError` 后 `_forward_hooks` 为空。脚本一次 leftover=1 是误测，独立复测关闭 |
...
| X8g | re-encode vs 生成 ids | 51 token 中 **8** 处不同（恰 `max_new=8`）。collect \(H\) 是表面文本二次前向 |
| X8h | 步前 vs 前缀前向 | `H[0]` 与 `hidden[44]` 及 `encode_text(text[:45])` 末 token **逐位相等**（\(\Delta=0\)）；\(\|H[0]-\mathrm{last}\|\approx 0.147\) |
| X8i | 三位置 | 行 0 \(\\|\Delta\\|\) pre/value/post \(\approx 0.156/0.147/0.158\)；`H is H_pre_step` |
| X9 | `leaks_target` | `[[0,10]]` limit=5 → True 且 `token_index=None` |
| X10 | `boundary_index(..., 0, "before")` | **None**（不可表达） |
| X11 | INLP 数学 vs \(\Pi_Z\) | \(h=[1,2,3]\)：\(h@P=[1.5,1.5,3]\)，`apply_swap` \(\neq\)，\(\max\|diff\|=2.36\)。hook 间谍：`out == in @ P`，`out ≠ swap` |
| X12 | 弱层 / 范数 | `select_weak_layer({0:0.05,1:0.9,2:0.8})=0`。缺范数 `c_rand_delta` → `ValueError`。三项范数锁成 \(2.4187\times 10^{-4}\) |
| X13 | StreamBank | `direction` 第一抽 113750710；第二抽 **1153799966**（`basis_seed`） |
...
| X14d | 无 `--dev-layer-scores` | `clayer_status=dev_scores_missing`；`clayer_transform` 缺；`clayer_outcomes` 四项 **null**；`relative.target/nontarget/invalid/task_correct` **null**。几何仍 `offline_hidden`。C-rand/INLP/rescue 仍 decode |
| X15 | thinking / 模板 | 未闭合 think → None；闭合 think 后数字 → 3；boxed → 7。`apply_model_template` / `load_frozen` / `clone_cache`：**CLI 与 `generate_task_trace` 无调用** |
| X16 | `clone_cache` | 本机无共享；`get_seq_length()=5`。生产路径不调用 |
...
| X22 | 生成区事件 | 6 条轨迹事件仅 `q`；`start` 45 或 53；问句长 36 或 44；**0** 条生成区 `p1`/`p2`。对问句跑 `parse_events` 会打出 p1/p2（本机累计 11），但 generate **不解析问句** |
| X23 | \(H\) 行距 | base–edit 0.00430；base–t0p 0.00778（**未被选**）；base–source 0.00832；`trace-edit` 与 `p2:2` 额外编辑 **全等**（同一默认编辑） |
| X24 | INLP hook 不是 swap | 间谍 `out_eq_inP=True`，`out_eq_swap=False`。CLI `mode="inlp"`。2 点拟合 `inlp_rank=30` |
| X25 | `ie_z` | 落盘 `0.0`（主/基线答案皆 ≠ donor `82`）。helper `ie_z(rescued, base)≈0.00237` **不等于**落盘值。CLI 用 \(g_{\mathrm{int}}-g_{\mathrm{base}}\)，`ie_z_g=target_follow` |
| X26 | 教师强制 | `target_assignment='\nq = 82'`（t0p 为 53）；`parse_status=constrained_target`；`correct=false`（金标 0）。intervene decode **不再**教师强制，4 token 经 `decode_ids`+`extract_answer` 得 `"9"` |
...
| 把 155 passed 写成声明冻结验收 | 交卷树已漂；作者写 153 |
...
| 台账项 | 作者摘要 | 本通道裁定 |
|---|---|---|
| 先前 r06/D7 生成区事件、有限步前 \(H\)、donor 身份、INLP/C-rand/rescue、`ie_z` | 须在 `81308124…` 上重核 | **独立关闭（指定路径，开审 MATCH 树）。** X8e、X14c、X22、X24、X25 |
...
| C6-M-01 / 假 P1 / PCA / Plus family | 他通道 | **本通道不裁定** |

## 6. 发现

### D10-00 — 开审冻结可锚定；交卷聚合已漂（元数据，非因果回归）

- **状态：** residual（冻结绑定） / non-defect（因果实现）
- **严重度：** process（连续通过计数）
- **文件 / 符号 / 行号：** `.planning/audits/round-10/VERSION.md` 声称 `81308124…`；交卷最后一次脚本输出 `929aa472…`
- **对应要求：** QA-01
- **证据：** X1、X2b。本通道未改 `src/`/`tests/`/`pyproject.toml`。§1 所列因果生产哈希中检=交卷。全量 pytest 155 ≠ 声称 153。
- **影响：** 科学点名项仍按开审 MATCH 树与未变因果字节成立。**不能**把交卷磁盘写成声明冻结已通过，**不能**开始连续通过。
- **建议：** 停写冻结集后再冻一轮；各通道绑同一聚合。

### D10-01 — 声称冻结 hash 开审已复现

- **状态：** closed / non-defect（开审）
- **严重度：** n/a
- **证据：** X1 开审。不因此放行 MODEL-01 或真实 C2。

### D10-02 — 只解析生成区；事件 `start ≥` 问句；问句 p1/p2 不是生成事件
...
### D10-03 — \(H\) 全有限；不可表达 `start=0` 跳过；无 NaN pre_step 行
...
- **影响：** r06「全 NaN donor / 不可表达行入槽」本机关闭。

### D10-04 — donor 来自 `source_value_pair` 的 `trace-base` / `trace-edit`，非全 NaN、非恒等 \(H\)

- **状态：** closed（点名配对） / residual（`same_value_diff_source` 未作第二 donor）
- **文件 / 符号 / 行号：** `cli.py` **384–397**（落盘 `trace_ids`）；**808–838**（`_load_source_value_pair` + `_pair_source_value`）；**841–844**（pair 优先于 t0p/finite fallback）
...
### D10-05 — INLP 是 \(h@P\)；C-rand `add_delta`；rescue `replace`；`ie_z` 为 target-follow \(g(Y)\)；有分数则弱层 `add_delta`

- **状态：** closed（点名接线） / residual（弱层 \(H\) 未采集；helper `ie_z` 仍是向量差；结局通道不是教师强制 \(q\)）
- **文件 / 符号 / 行号：** `collect.py` **189–208**；`cli.py` **912–981、960–964**；`interventions.py` **52–67、88–89**
- **对应要求：** CAUSAL-01；INLP-01；IE-01
- **证据：** X11、X12、X14c、X14d、X24、X25。hook 间谍 `out = in @ P` 且 \(\neq \Pi_Z\) swap。CLI：主 `pi_z_swap`；INLP `mode=inlp`；C-rand `add_delta(cr["delta"])`；rescue `replace(rescued["matched"])`；有分数时层 **0** `add_delta(cl["delta"])` 并写 `dev_weak_layer_decode`；无分数不写弱层 decode，相对四项 null。落盘 `ie_z=0.0` ≠ helper 0.00237；`ie_z_g=target_follow`。\(g=1\) 当 `extract_answer(generated)==donor_ans`。本机 donor_ans=82、主/基线皆非 82。
...
### D10-06 — 几何 `timing` 保持 `offline_hidden`，未写成 live `pre_step`

- **状态：** closed
- **文件 / 符号 / 行号：** `cli.py` **911、1003**；`collect.py` **227**（hook 内部 `timing`）
...
### D10-07 — Prefill：真实 32-d tiny hidden 接受；`0`/`True`/`1.0`/`[0]`/`[]` 拒绝
...
### D10-08 — 教师强制 `q` 诚实为 `constrained_target`；不得写 MODEL-01 / §4.1

- **状态：** closed（诚实标签） / non-defect（tiny 边界） / residual（模板未接入）
- **文件 / 符号 / 行号：** `generate.py` **72–98、157–158、195–201**；`adapters.py` **32–42**
- **证据：** X15、X26、X27。`append_target_assignment` 写 `\nq = ` 再从 logits 采两位数字，注释写明 Not gold values。6 条 `parse_status=constrained_target`，`correct=false`。金标 0，写出 82/53。`apply_model_template` 不被 CLI/generate 调用。卡片 `think_ids` 只在未调用的 `load_frozen` 路径。
- **影响：** 本机可解析接口成立。把这条轨迹写成自然 CoT、官方模板对齐或 MODEL-01 **是缺陷**；当前代码与元数据没有那样写。

### D10-09 — nontarget/invalid 字段已接线；本机取值不能当机制结果

- **状态：** closed（schema / 相对对照） / residual（通道与 identifiability）
- **文件 / 符号 / 行号：** `cli.py` **949–957、982**；`interventions.py` **98–115**
...
| U-03 | 无约束 4-token 下 INLP/swap/add/replace 曾产出同一 `generated_ids` | 间谍已证变换数学不同；tiny + \(\\|\Delta\\|\\approx 2.4\\times 10^{-4}\) 不足以改采样。不升格为「INLP 仍是 swap」 |
...
| U-06 | `extra_prefill_tokens` 恒 32 | 修复预算/截断，见 D10-07 |
...
相对 r08 已关闭、本轮不再当缺陷：INLP decode 仍走 `apply_swap`；donor 只吃 t0p；`ie_z` 只是隐均值；C-rand 无 decode；Prefill 接受 `0`/`[0]`。

## 10. 通道结论

**D 通道通过（PASS）。开审冻结：HASH_MATCH `81308124…`。交卷冻结：HASH_MISMATCH（最后一次 `929aa472…`）。**

指定 scientific `prepare`→`collect tiny`→`intervene tiny`→`repair tiny` 在开审 MATCH 树上独立复跑，点名项成立：
