...
点名猎项：无静默 48 字符截断，来源问句须含完整改写式（本夹具 XOR/`*` 落地则为 `* src_b`）；XOR 保留叶 *a*、追加等值 *b*，不是 rename；配对只读给定 `in-dir/edits.jsonl`，不依赖兄弟 `prep`；跳过全 NaN \(H\) 行；`pre_step` 在 `start=0` 不可表达；`pi_z_swap` / INLP \(h@P\) / `add_delta` / `replace`；C-rand/C-layer 锁到主干预实际范数；仅生成区事件且 `parse_region=generated`；Prefill 拒绝标量/bool/`[0]`；教师强制诚实标 `constrained_target`，不是 §4.1 / MODEL-01；`IsolatedExecutor` ≠ `ChildProcessExecutor`。

独立性声明：未阅读 `.planning/audits/round-18/` 下除 `VERSION.md` 与本文件外的其他通道报告。只把 `VERSION.md` 与 `.planning/audits/ISSUES.md` 当作冻结声明 / 作者台账，不把作者 `local close` 当关闭。对照读了本通道 round-15 报告仅作版式与猎单结构，不作本轮证据。未修改 `src/`、`tests/` 或 `pyproject.toml`。绿测试不是论文正确性。微型随机权重是本地接口证明，**不是** MODEL-01。Gate 未注册与 `scientific_conclusion=None` 不记缺陷。约束 `\nq = <digit>` **不是** §4.1 自然 CoT。

对照原文：论文 §2.4（\(H'=H_b+\Pi_Z(H_d-H_b)\)、C-rand/C-layer 同批次）、§4.1（自然 CoT，本机不得冒称）、§4.2 / 第 5–6 节（三位置；前瞻交换）、第 6/8 节（来源—数值 / Knock-out / Rescue）；GOAL MODEL-01 / CAUSAL-01 / CAUSAL-02 / POS-01 / INLP-01 / IE-01 / EXEC-01 / REPAIR-01；`docs/EXPERIMENT_PROTOCOL.md` §4。tiny 路径只验收接口与接线，不验收 MODEL-01。

**先行结论：** 按 `.planning/audits/round-18/VERSION.md` 原文脚本复算，开审与交卷均为 **HASH_MATCH** `3d0f1c1025d501509a88023df5f14fe32392c38f28657fed2cb906bdb7c0952f`（61 文件，0 CRLF）。指定 scientific 路径在无 `prep` 兄弟的 `stage_a`/`stage_b` 上独立复跑，点名猎项全部成立。未发现需改冻结集的本通道缺陷。
...
| 审查时间 | 2026-09-21（Asia/Shanghai）；开审约 02:59，交卷约 03:12 |
| 声称冻结 hash | `3d0f1c1025d501509a88023df5f14fe32392c38f28657fed2cb906bdb7c0952f`（`.planning/audits/round-18/VERSION.md`；61 文件；POSIX relpath + NUL + bytes） |
| 开审复算 | **HASH_MATCH。** 脚本逐字：`src/reasoning_diff/**/*.py` 排序 + `tests/**/*.py` 排序 + `pyproject.toml` → **61** 文件 → `3d0f1c1025d501509a88023df5f14fe32392c38f28657fed2cb906bdb7c0952f` |
...
| 作者 pytest 声称 | 162 passed。本机全量 **162 passed / 34.87s / exit 0**。绿只锁回归，不是论文正确性 |
| 明确未读 | `.planning/audits/round-18/` 下除 `VERSION.md` 与本文件外的其他通道报告 |

因果生产文件 SHA-256（交卷 = 开审）：`cli.py` `9431b7768fda6a3c`（61646 B / 1260 行）、`edits.py` `1e5b97d63a62ab78`（13773 B / 365）、`collect.py` `83658cd8ba879f20`（261）、`generate.py` `6e6040394ac641c7`（200）、`tiny.py` `c75d0f5325766612`（120）、`features.py` `0a9f0b8beb0185ae`（39）、`adapters.py` `c1992624026a1bf0`（42）、`tokenize.py` `b0cc8974af1010fa`（31）、`interventions.py` `0ffdbb8805be7649`（115）、`repair.py` `f76ff9998b9a6b17`（232）、`events.py` `290a4fd676ac0814`（274）、`executor.py` `481d6ed597c93e78`（109）、`rng.py` `2098c2c72a852eaa`（53）。全部 0 CRLF。

复跑目录（本通道自建）：`.planning/audits/round-18/_d_scratch/{stage_a,stage_b,stage_int,stage_int_dev,stage_rep,stage_off}`。兄弟名只有这些；**没有** `prep` / `prepare` / `s-prep`。`stage_a` = prepare，`stage_b` = collect（`--in-dir stage_a`），intervene / repair 的 `--in-dir` 是 `stage_b`。
...
| `src/reasoning_diff/edits.py` | 109–365 | `apply_value_edit`；`apply_rename_edit`；`apply_alt_source_same_value`（保留叶 *a*、固定名 `src_b`、父母改读 *b*）；`make_source_value_pair` |
| `src/reasoning_diff/repair.py` | 1–232 | `_hidden_is_prefill` 拒绝标量/bool/`[0]`；`execute_repair_tiny`；`run_repair` |
| `src/reasoning_diff/cli.py` | 187–226、257–551、687–700、817–1062 | `_domain_edit` / `_allowed_edits`；prepare（含 `trace-source`）；collect 复制 `edits.jsonl`；`_load_source_value_pair` 只读 `in-dir/edits.jsonl`；`_pair_source_value` 跳过非有限行；intervene / repair |
...
| ID | 检查 | 结果 |
|---|---|---|
| X1 | VERSION 脚本逐字复算 | **开审 HASH_MATCH** 61 / `3d0f1c10…c0952f`（0 CRLF）。**交卷 HASH_MATCH** 同一 digest |
| X2 | 全量 pytest | **162 passed / 34.87s / exit 0**，与作者数字一致。仍不是论文正确性 |
...
| X12 | C-rand / C-layer 范数与弱层 | 缺范数仍 `ValueError`（`C-rand requires the actual main-intervention norm`）。`select_weak_layer({0:0.05,1:0.9,2:0.8})=0`。独立重算 `crand_norm=clayer_norm=0.002791401781066686`，与 `stage_int_dev` 落盘逐位一致；主干预范数 `0.0027914017810666864`（`scale_to_norm` 后对照锁到该值） |
...
| X22 | 事件跨度 / 无 48 截断 | 生成区 `q` 跨度 45–（source 58）；`prefix_truncated=false`。6/6 `start ≥ len(prompt_text)` 且 `start ≥ prompt_len`。`generate.py` 无 `[:48]`；`>96` raise。source `prompt_len=49`（若仍静默切 48 会丢掉 `* src_b` 尾部） |
...
| X26 | 问句单独解析 | 对 6 条 `prompt_text` 跑 `parse_events` 会打出 p1/p2（source 问句为 p1/`p2`/`src_b`）。generate **不解析问句**，故这些不是生成事件 |
...
| 把 162 passed 写成 Goal / MODEL-01 验收 | 绿测试不是论文正确性 |
...
| D14-05 | tiny 不再 48 字符截断 | **指定路径独立关闭。** X22。source 问句 49 字符且含完整 `* src_b` |
| A13-01 | 来源臂不是 rename-only；intervene 优先 `trace-source` | **指定路径独立关闭。** X14c/d、X21。有 source donor 时不是 value-edit-only |
| A14-03 / A14-04 / A13-02 / A13-03 / A12-03 / A10-04 | fit / excess / 未知行为 / sham / tasks 查找 / Plus 锁 | **不裁定。** 非本通道猎项 |
| pending_server | 真实权重 / 50 条 C2 / 官方 CoT | **同意未关。** 约束 `q` 不是 §4.1；tiny 不是 MODEL-01 |
| Gate 未注册 | 非缺陷 | **同意。** 不记缺陷 |

## 6. 发现

### D18-01 — 无静默 48 字符截断；source 问句含完整 `* src_b`

- **状态：** closed
- **严重度：** n/a
- **文件 / 符号 / 行号：** `generate.py` **124–125**（`len(prompt_ids)>96` raise；全文无 `[:48]`）；**122–123、183**（整段 `task.question` 编码）
- **对应要求：** 点名猎项「No silent 48-char cap」；作者 D14-05
- **证据：** X8e、X21、X22。source `prompt_text='p1 = 4. p2 = 0 src_b = 0. What is q = p1 * src_b?'`，长度 **49**，`prompt_len=49`，尾部 `t is q = p1 * src_b?`。若仍静默 `[:48]`，会切掉 `* src_b` 的尾字符。`src/reasoning_diff` 内无字面量 `48`。`tiny.max_position_embeddings=128`，本条 64 token 全文未触顶
- **影响：** 指定路径不再用 48 字符丢掉改写式。这不是自然 CoT

### D18-02 — XOR 保留 *a*、追加 *b*，不是 rename；配对不依赖兄弟 `prep`

- **状态：** closed（点名配对与图构造） / residual（夹具级换源，不是 50 条独立计算源）
- **严重度：** n/a / residual
- **文件 / 符号 / 行号：** `edits.py` **219–292**（`apply_alt_source_same_value`：固定 `src_b`、保留原叶、父母改读 *b*、目标值不变）；**295–304**（`make_source_value_pair` 不调 rename）；`cli.py` **521–524**（collect 复制 `edits.jsonl`）；**817–821**（只读 `in-dir/edits.jsonl`）；**834–846**（键序 `same_value_diff_source` 先于 `same_source_diff_value`；非有限行不入 index）
- **对应要求：** CAUSAL-02；作者 F15-01 / A14-02 / A13-01
- **证据：** X14c/d/f、X21、X28。
...
- **影响：** r15 F15-01「兄弟目录名 `prep` 才能配对」在本冻结指定路径上不成立

### D18-03 — \(H\) 全有限；全 NaN 行跳过；不可表达 `start=0` 跳过

- **状态：** closed
- **严重度：** n/a
- **文件 / 符号 / 行号：** `collect.py` **50–68**；`events.py` **251–257**；`cli.py` **830–832、854–873**
- **对应要求：** 可表达步前入 \(H\)；点名「Skip all-NaN H rows」「pre_step inexpressible at start=0」
...
### D18-04 — INLP 是 \(h@P\)；C-rand `add_delta`；rescue `replace`；范数锁到主干预
...
- **证据：** X11、X12、X14c、X14d、X23、X24。hook 间谍 `out = in @ P`。五次 decode 键集合恰好 `{(pi_z_swap,1),(add_delta,1),(add_delta,0),(inlp,1),(replace,1)}`。C-rand / C-layer 范数与独立 `c_rand_delta` / `c_layer_delta` 对拍。rescue 是 `replace`，不是再 swap。落盘 `ie_z=0.0` ≠ helper \(-3.07\times 10^{-4}\)；`ie_z_g=target_follow`。本机 donor_ans=82、主/基线/对照皆为 9
...
### D18-05 — 仅生成区事件；几何 `timing` 保持 `offline_hidden`

- **状态：** closed
- **严重度：** n/a
- **文件 / 符号 / 行号：** `generate.py` **147–189**；`cli.py` **920、1012**；`collect.py` **227**
...
### D18-06 — Prefill：真实 32-d tiny hidden 接受；标量/bool/`[0]` 拒绝
...
### D18-07 — 教师强制诚实为 `constrained_target`；不得写 MODEL-01 / §4.1
...
### D18-08 — `IsolatedExecutor` ≠ `ChildProcessExecutor`
...
| U-05 | intervene 前缀仍 `ids[:64]` | 本夹具 base 前缀 45 token，`prefix_truncated=false`；猎项针对 generate 的 48 字符静默切，未触发 |
| U-06 | C-layer 把读出层 \(\Delta\) 加到弱层 residual | 猎项只要求 `c_layer_delta`+弱层 `add_delta`，已对拍 |
| U-07 | `extra_prefill_tokens` 恒 32 且不落盘 hidden | 旗标与拒绝规则已核 |
...
| N-11 | 过程 | 开审与交卷均为 HASH_MATCH，可签在声称快照 `3d0f1c10…` 上 |
...
| N-20 | 关闭（本轮猎项） | 无 48 截断；keep-*a* add-*b*；无 `prep` 配对；有限 \(H\)；跳过 NaN；`start=0` 跳过；INLP=\(h@P\)；C-rand/rescue/C-layer；生成区事件；Prefill 拒绝；`constrained_target`；执行器不等价 |
...
**D 通道通过（PASS）。冻结：HASH_MATCH `3d0f1c1025d501509a88023df5f14fe32392c38f28657fed2cb906bdb7c0952f`（61 文件）。指定猎项在独立复跑上关闭。tiny 不是 MODEL-01；自然 CoT 不是本通道通过条件。不宣布 Goal 完成。本通道单独不得启动连续通过计数。**

开审与交卷复算冻结均为 `3d0f1c10…c0952f`。在 `.planning/audits/round-18/_d_scratch/` 对 `tests/fixtures/t1_tiny.json` 跑 scientific `prepare --split-fractions 0.4 0.15 0.1 0.1 0.1 0.15` → `collect --backend tiny --weight-seed 0` → `intervene --backend tiny` → `intervene --dev-layer-scores 0.05 0.9 0.8` → `repair --eval-mode scientific --mask task_oracle`。目录名为 `stage_a`/`stage_b`，**没有**兄弟 `prep`。核验：

1. **无 48 截断：** source 问句 49 字符，完整 `What is q = p1 * src_b?`。`generate.py` 超 96 则 raise。
2. **keep-*a* add-*b*：** 保留 `p2`，追加 `src_b`，父母 `{p1,src_b}`；不是 rename（rename 丢掉 `p2`）。
3. **无 prep 配对：** collect 把 `edits.jsonl` 拷进 `stage_b`；intervene 只读该文件；donor=`same_value_diff_source` 行 `(0,3)`。
4. **有限 \(H\) / NaN / start=0：** `(6,32)` 全有限；source 行全 NaN 则改选 value 臂；全 NaN 不配对；`start=0` → `no_event`。
5. **INLP / 对照：** `mode=inlp` 为 \(h@P\)；C-rand/C-layer `add_delta` 范数与独立重算落盘一致；rescue `replace`。
6. **生成区事件：** 6/6 `parse_region=generated`，仅 `q`，`start` 不落入 prompt。
7. **Prefill：** `0` / `True` / `[0]` 拒绝；scientific repair k=1..5 有真实 32 维 hidden。
8. **教师强制：** 6/6 `constrained_target`；不是 §4.1，也不是 MODEL-01。
9. **执行器：** `ChildProcessExecutor` 不是 `IsolatedExecutor`。
