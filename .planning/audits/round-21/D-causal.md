...
点名猎项：XOR 保留叶 *a*、追加等值 *b*；重叠 rename 必须同时替换；配对只读给定 `in-dir/edits.jsonl`；INLP \(h@P\) / `pi_z_swap` / `add_delta` / `replace`；C-rand/C-layer 锁到主干预实际范数；仅生成区事件且 `parse_region=generated`；Prefill 拒绝标量/bool/`[0]`；教师强制诚实标 `constrained_target`，不是 §4.1 / MODEL-01；`IsolatedExecutor` ≠ `ChildProcessExecutor`；70 字题干的 hook `prompt_ids` 长度 >64，不得静默切 64；超 96 必须 raise 不得 slice。

独立性声明：未阅读 `.planning/audits/round-21/` 下除 `VERSION.md` 与本文件外的其他通道报告。只把 `VERSION.md` 与 `.planning/audits/ISSUES.md` 当作冻结声明 / 作者台账，不把作者 `local close` 当关闭。对照读了本通道 round-20 报告仅作版式与猎单结构，不作本轮证据。未修改 `src/`、`tests/` 或 `pyproject.toml`。绿测试不是论文正确性。微型随机权重是本地接口证明，**不是** MODEL-01。Gate 未注册与 `scientific_conclusion=None` 不记缺陷。约束 `\nq = <digit>` **不是** §4.1 自然 CoT。作者 F20-02（`cmd_calibrate` 兄弟 `lab`）与 F19-06（scientific fit §8 行）不在本猎单，不裁定。
...
**先行结论：** 按 `.planning/audits/round-21/VERSION.md` 原文脚本复算，开审与交卷均为 **HASH_MATCH** `5097c8310b023492eda617986ce11a956309a6452cc00121e0e90167a6f7aceb`（61 文件，0 CRLF）。指定 scientific 路径在无 `prep` 兄弟的 `stage_a`/`stage_b` 上独立复跑。70 字题干的独立三路间谍（`intervene_hidden_decode` 的 `prompt_ids`、`decode_loop` 张量宽、`resid_post` 首次前向 `seq`）均为 **79**，全部 >64，无一被切到 64；落盘 `prefix_n=79` 与间谍一致。97 token 前缀与 97/127 字问句 raise。点名猎项全部成立。未发现需改冻结集的本通道缺陷。
...
| 审查时间 | 2026-09-21（Asia/Shanghai）；开审约 03:27，交卷约 03:50 |
| 声称冻结 hash | `5097c8310b023492eda617986ce11a956309a6452cc00121e0e90167a6f7aceb`（`.planning/audits/round-21/VERSION.md`；61 文件；POSIX relpath + NUL + bytes） |
| 开审复算 | **HASH_MATCH。** 脚本逐字：`src/reasoning_diff/**/*.py` 排序 + `tests/**/*.py` 排序 + `pyproject.toml` → **61** 文件 → `5097c8310b023492eda617986ce11a956309a6452cc00121e0e90167a6f7aceb`（0 CRLF） |
...
| 作者 pytest 声称 | 167 passed。本机全量 **167 passed / 23.89s / exit 0**。绿只锁回归，不是论文正确性 |
| 明确未读 | `.planning/audits/round-21/` 下除 `VERSION.md` 与本文件外的其他通道报告 |

因果生产文件 SHA-256（交卷 = 开审）：`cli.py` `cc5eae53d7be27eb`（61734 B / 1268 行）、`edits.py` `cc8e2d23592a3ab8`（13905 B / 367）、`collect.py` `83658cd8ba879f20`（261）、`generate.py` `6e6040394ac641c7`（200）、`tiny.py` `c75d0f5325766612`（120）、`features.py` `0a9f0b8beb0185ae`（39）、`adapters.py` `c1992624026a1bf0`（42）、`tokenize.py` `b0cc8974af1010fa`（31）、`interventions.py` `0ffdbb8805be7649`（115）、`repair.py` `f76ff9998b9a6b17`（232）、`events.py` `290a4fd676ac0814`（274）、`executor.py` `481d6ed597c93e78`（109）、`rng.py` `2098c2c72a852eaa`（53）。全部 0 CRLF。相对作者作废的 r20 树，本通道可见的冻结差在 `cli.py`（intervene 写 `prefix_n=len(ids)`；1267→1268 行）。r20 A–E PASS 不转移到本树。本通道仍对当前磁盘独立复跑。

复跑目录（本通道自建）：`.planning/audits/round-21/_d_scratch/{stage_a,stage_b,stage_int,stage_int_dev,stage_rep,stage_off,stage_no_edits,stage_70a,stage_70b,stage_70int}`。兄弟名只有这些加 `hunts.py` / `hunts.json` / `t1_70.json`；**没有** `prep` / `prepare` / `s-prep`。`stage_a` = prepare，`stage_b` = collect（`--in-dir stage_a`），intervene / repair 的 `--in-dir` 是 `stage_b`。70 字攻击用 `stage_70a`/`stage_70b`/`stage_70int`，不占用指定名。
...
| ID | 检查 | 结果 |
|---|---|---|
| X1 | VERSION 脚本逐字复算 | **开审 HASH_MATCH** 61 / `5097c8310b023492eda617986ce11a956309a6452cc00121e0e90167a6f7aceb`（0 CRLF）。**交卷 HASH_MATCH** 同一 digest |
| X2 | 全量 pytest | **167 passed / 23.89s / exit 0**，与作者数字一致。仍不是论文正确性 |
| X3 | `apply_swap` 标准正交基 | \(H_b=[1,0],H_d=[0,1],Z=e_2\) → \(H'=[1,1]\)。全空间 \(I_2\) 则 \(H'=H_d=[0,1]\) |
| X4 | tiny qwen2：`resid_post` 层 1 后 KV | 层 0/1 \(\Delta K=\Delta V=0\)；层 2 \(\Delta K_{\max}=0.154\)、\(\Delta V_{\max}=0.349\)，且 **仅 last token**（earlyK=0） |
...
| X11 | INLP / rescue 算法 | 随机向量上 \(h@P\) 与 `apply_swap` 最大差 **0.445**（不是同一变换）。库函数 `ie_z(H_3,H_0)\approx -3.07\times 10^{-4}`；**CLI tiny 路径不写该值** |
...
| X24 | INLP hook 不是 swap | 独立 residual 间谍：`out == in @ P`（matmul 差 **0**），`out ≠ in`（差 0.043）。CLI `mode="inlp"`，无 donor。2 点拟合 `inlp_rank=30` |
...
| 把 167 passed 写成 Goal / MODEL-01 验收 | 绿测试不是论文正确性 |
...
### D21-01 — 无静默 generate 48 截断；超长 prompt raise
...
### D21-02 — 独立间谍 hook `prompt_ids`：70 字前缀不切开；超长 raise 不 slice
...
- **证据：** X14c、X22b。冻结树无 `ids[:64]` / `[:64]` / `[:48]`。
  - `_tiny_prefix_ids("a"*65)` 长度 65；`("x"*70)` 长度 70；`("c"*96)` 长度 96
  ...
  - **三路运行时间谍（本通道自挂，不读作者测试）：**
    1. 包装 `collect.intervene_hidden_decode`：五次 `len(prompt_ids)=79`
...
### D21-03 — XOR 保留 *a*、追加 *b*；重叠 rename 同时替换；配对不依赖兄弟 `prep`
...
### D21-04 — \(H\) 全有限；全 NaN 行跳过；不可表达 `start=0` 跳过
...
### D21-05 — INLP 是 \(h@P\)；C-rand `add_delta`；rescue `replace`；范数锁到主干预
...
- **证据：** X11、X12、X14c、X14d、X23、X24。residual 间谍 `out = in @ P`（matmul 差 0）。五次 decode 键集合恰好 `{(pi_z_swap,1),(add_delta,1),(add_delta,0),(inlp,1),(replace,1)}`。C-rand / C-layer 范数与独立 `c_rand_delta` / `c_layer_delta` 对拍。rescue 是 `replace`，不是再 swap。落盘 `ie_z=0.0` ≠ helper \(-3.07\times 10^{-4}\)；`ie_z_g=target_follow`。本机 donor_ans=82、主/基线/对照皆为 9
...
### D21-06 — 仅生成区事件；几何 `timing` 保持 `offline_hidden`
...
### D21-07 — Prefill：真实 32-d tiny hidden 接受；标量/bool/`[0]` 拒绝
...
### D21-08 — 教师强制诚实为 `constrained_target`；不得写 MODEL-01 / §4.1
...
### D21-09 — `IsolatedExecutor` ≠ `ChildProcessExecutor`
...
| N-11 | 过程 | 开审与交卷均为 HASH_MATCH，可签在声称快照 `5097c8310b02…a6f7aceb` 上 |
...
| N-20 | 关闭（本轮猎项） | 无 generate 48 截断；无 intervene 64 截断（70 字 hook `prompt_ids`=79 不切开；超长 raise）；重叠 rename 同时替换；keep-*a* add-*b*；无 `prep` 配对；有限 \(H\)；跳过 NaN；INLP=\(h@P\)；C-rand/rescue/C-layer；生成区事件；Prefill 拒绝；`constrained_target`；执行器不等价 |
...
**D 通道通过（PASS）。冻结：HASH_MATCH `5097c8310b023492eda617986ce11a956309a6452cc00121e0e90167a6f7aceb`（61 文件）。指定猎项在独立复跑上关闭。tiny 不是 MODEL-01；自然 CoT 不是本通道通过条件。不宣布 Goal 完成。本通道单独不得启动连续通过计数。**

开审与交卷复算冻结均为 `5097c8310b02…a6f7aceb`。在 `.planning/audits/round-21/_d_scratch/` 对 `tests/fixtures/t1_tiny.json` 跑 scientific `prepare --split-fractions 0.4 0.15 0.1 0.1 0.1 0.15` → `collect --backend tiny --weight-seed 0` → `intervene --backend tiny` → `intervene --dev-layer-scores 0.05 0.9 0.8` → `repair --eval-mode scientific --mask task_oracle`。目录名为 `stage_a`/`stage_b`，**没有**兄弟 `prep`。另用 pad 到 70 字的夹具跑 `stage_70a`→`stage_70b`→`stage_70int`，并自挂 hook/`decode_loop`/`resid_post` 间谍。核验：
