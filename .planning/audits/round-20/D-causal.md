...
**先行结论：** 按 `.planning/audits/round-20/VERSION.md` 原文脚本复算，开审与交卷均为 **HASH_MATCH** `dc36f21713f2a8e021d44c992681c13aafd76ccd93a545c331fd984f787fbebb`（61 文件，0 CRLF）。指定 scientific 路径在无 `prep` 兄弟的 `stage_a`/`stage_b` 上独立复跑；70 字前缀 CLI 间谍得到 79 token、未被切到 64；97 token raise。点名猎项全部成立。未发现需改冻结集的本通道缺陷。
...
| 审查时间 | 2026-09-21（Asia/Shanghai）；开审约 03:18，交卷约 03:40 |
| 声称冻结 hash | `dc36f21713f2a8e021d44c992681c13aafd76ccd93a545c331fd984f787fbebb`（`.planning/audits/round-20/VERSION.md`；61 文件；POSIX relpath + NUL + bytes） |
| 开审复算 | **HASH_MATCH。** 脚本逐字：`src/reasoning_diff/**/*.py` 排序 + `tests/**/*.py` 排序 + `pyproject.toml` → **61** 文件 → `dc36f21713f2a8e021d44c992681c13aafd76ccd93a545c331fd984f787fbebb` |
...
| 作者 pytest 声称 | 167 passed。本机全量 **167 passed / 23.96s / exit 0**。绿只锁回归，不是论文正确性 |
| 明确未读 | `.planning/audits/round-20/` 下除 `VERSION.md` 与本文件外的其他通道报告 |

因果生产文件 SHA-256（交卷 = 开审）：`cli.py` `95c58b934e1aceeb`（61685 B / 1267 行）、`edits.py` `cc8e2d23592a3ab8`（13905 B / 367）、`collect.py` `83658cd8ba879f20`（261）、`generate.py` `6e6040394ac641c7`（200）、`tiny.py` `c75d0f5325766612`（120）、`features.py` `0a9f0b8beb0185ae`（39）、`adapters.py` `c1992624026a1bf0`（42）、`tokenize.py` `b0cc8974af1010fa`（31）、`interventions.py` `0ffdbb8805be7649`（115）、`repair.py` `f76ff9998b9a6b17`（232）、`events.py` `290a4fd676ac0814`（274）、`executor.py` `481d6ed597c93e78`（109）、`rng.py` `2098c2c72a852eaa`（53）。全部 0 CRLF。本轮冻结相对上一审查树的 digest 变化不在这 13 个生产文件上（作者台账指向测试锁 70 字前缀与 fit §8 行）；本通道仍对当前磁盘独立复跑，不转移旧 PASS。

复跑目录（本通道自建）：`.planning/audits/round-20/_d_scratch/{stage_a,stage_b,stage_int,stage_int_dev,stage_rep,stage_off,stage_no_edits,stage_70a,stage_70b,stage_70int}`。兄弟名只有这些加 hunt/analyze 脚本与 `t1_70.json`；**没有** `prep` / `prepare` / `s-prep`。`stage_a` = prepare，`stage_b` = collect（`--in-dir stage_a`），intervene / repair 的 `--in-dir` 是 `stage_b`。70 字攻击用 `stage_70a`/`stage_70b`/`stage_70int`，不占用指定名。
...
支持性阅读：论文 §2.4 / 第 6、8 节；`docs/EXPERIMENT_PROTOCOL.md` §4；`.planning/REQUIREMENTS.md` 上列要求；`docs/CURSOR_GOAL_PROMPT.md` §5.10–5.15。未覆盖且不得默认通过：真实权重加载、CUDA/长链 KV、官方 tokenizer JSON、50 条来源解耦 C2 同批次、自然 CoT。未逐行审计：`analysis.py`、`probes/bilinear.py`、`transfer.py`、`measure.py`、`splits.py`（他通道）。`cli.py` 校准/fit 段属其他通道；作者 F19-06 / 校准兄弟 `lab` 不在本猎单。

## 3. 已执行检查与结果

| ID | 检查 | 结果 |
|---|---|---|
| X1 | VERSION 脚本逐字复算 | **开审 HASH_MATCH** 61 / `dc36f21713f2a8e021d44c992681c13aafd76ccd93a545c331fd984f787fbebb`（0 CRLF）。**交卷 HASH_MATCH** 同一 digest |
| X2 | 全量 pytest | **167 passed / 23.96s / exit 0**，与作者数字一致。仍不是论文正确性 |
...
| X22b | **无 intervene 64 截断；70 字不切开；超长 raise** | `src/reasoning_diff` 无 `ids[:64]` / `[:64]` / `[:48]`。`_tiny_prefix_ids` 默认 `limit=96`：65 / 70 / 96 token 前缀原样返回（旧 64-cap 会切开 65 与 70）；97 token → `ValueError: tiny intervene prefix exceeds context; refuse truncated prefixes`。独立 70 字夹具：问句 pad 到 70，`stage_70a` 六条 `prompt_text` 最短 **70**（source 臂 83）；intervene 间谍 `_tiny_prefix_ids` 收到 **79** 字符 / **79** ids（问句 70 + 生成 8 + 换行），`sliced_to_64=false`，落盘 `prefix_truncated=false`，`donor_kind=same_value_diff_source`。本夹具 base 前缀 45 token |
...
| X26 | 问句单独解析 | 对 6 条 `prompt_text` 用 **base** task 跑 `parse_events` 会打出 p1/p2。source 问句用 **source** task 解析得 `p1/p2/src_b`。generate **不解析问句**，故这些不是生成事件 |
...
| `cmd_calibrate` 兄弟 `lab` 行走 | 非本通道猎项。不裁定 |
...
| 台账项 | 作者摘要 | 本通道裁定 |
|---|---|---|
| F19-06 | scientific fit 断言要求四条 §8 行存在、`refused_not_section8`、无 `score` | **不裁定。** 非本通道猎项 |
| F19-07 | CLI 锁 70 字 intervene 前缀；`cmd_calibrate` 源码不含兄弟 `lab` | **70 字前缀：指定路径独立关闭。** X22b。校准兄弟行走 **不裁定** |
...
### D20-01 — 无静默 generate 48 截断；超长 prompt raise
...
### D20-02 — 无静默 intervene 64 截断；70 字前缀不切开；超长 raise 不 slice
...
- **对应要求：** 点名猎项「70-char prefix intervene；no silent 64-cap；overlong must raise」；作者 F18-08 / F19-07（前半）
- **证据：** X14c、X22b。冻结树无 `ids[:64]` / `[:64]` / `[:48]`。
  ...
  - intervene 运行时间谍：`prefix_chars=79`，`n_ids=79`，`limit=96`，`sliced_to_64=false`（79>64；旧 cap 必切开）
  - `stage_70int` 落盘 `prefix_truncated=false`，`status=prospective_decode`
- **影响：** 指定 intervene 路径不再静默切 64。70 字攻击成立。collect `_hidden_at_layer` 仍按 128 切（见 U-05），不是本猎项

### D20-03 — XOR 保留 *a*、追加 *b*；重叠 rename 同时替换；配对不依赖兄弟 `prep`
...
### D20-04 — \(H\) 全有限；全 NaN 行跳过；不可表达 `start=0` 跳过
...
### D20-05 — INLP 是 \(h@P\)；C-rand `add_delta`；rescue `replace`；范数锁到主干预
...
### D20-06 — 仅生成区事件；几何 `timing` 保持 `offline_hidden`
...
### D20-07 — Prefill：真实 32-d tiny hidden 接受；标量/bool/`[0]` 拒绝
...
### D20-08 — 教师强制诚实为 `constrained_target`；不得写 MODEL-01 / §4.1
...
### D20-09 — `IsolatedExecutor` ≠ `ChildProcessExecutor`
...
| U-11 | 70 字攻击的 intervene 前缀是 `text[:event.start]`=79（含 8 乱码 + 换行），不是刚好 70 | 猎项是「≥70 且不被 64-cap 切开」。79>64 且间谍未切片 |
...
| N-11 | 过程 | 开审与交卷均为 HASH_MATCH，可签在声称快照 `dc36f21713f2a8e0…` 上 |
...
| N-20 | 关闭（本轮猎项） | 无 generate 48 截断；无 intervene 64 截断（70 字不切开；超长 raise）；重叠 rename 同时替换；keep-*a* add-*b*；无 `prep` 配对；有限 \(H\)；跳过 NaN；INLP=\(h@P\)；C-rand/rescue/C-layer；生成区事件；Prefill 拒绝；`constrained_target`；执行器不等价 |
...
**D 通道通过（PASS）。冻结：HASH_MATCH `dc36f21713f2a8e021d44c992681c13aafd76ccd93a545c331fd984f787fbebb`（61 文件）。指定猎项在独立复跑上关闭。tiny 不是 MODEL-01；自然 CoT 不是本通道通过条件。不宣布 Goal 完成。本通道单独不得启动连续通过计数。**

开审与交卷复算冻结均为 `dc36f21713f2a8e0…787fbebb`。在 `.planning/audits/round-20/_d_scratch/` 对 `tests/fixtures/t1_tiny.json` 跑 scientific `prepare --split-fractions 0.4 0.15 0.1 0.1 0.1 0.15` → `collect --backend tiny --weight-seed 0` → `intervene --backend tiny` → `intervene --dev-layer-scores 0.05 0.9 0.8` → `repair --eval-mode scientific --mask task_oracle`。目录名为 `stage_a`/`stage_b`，**没有**兄弟 `prep`。另用 pad 到 70 字的夹具跑 `stage_70a`→`stage_70b`→`stage_70int`。核验：

1. **无 generate 48 截断：** source 问句 49 字符，完整 `What is q = p1 * src_b?`。`generate.py` 超 96 则 raise。
2. **无 intervene 64 截断 / 70 字攻击：** `_tiny_prefix_ids` 65/70/96 token 不切片；97 token raise。无 `ids[:64]`。70 字 CLI 间谍 79 ids，未切到 64。
